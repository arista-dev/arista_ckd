...
@section('content')
    ...
    <form method="POST" action="{{ route('inspection.update', $inspection->id) }}" enctype="multipart/form-data">
        @csrf
                            ...
                            @foreach ($inspection->items as $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @else
                                            <input type="number" name="actual_qty_{{ $item->component_code }}"
                                    ...
                                    {{-- Short Qty --}}
                                    <td class="text-center" id="short-{{ $item->component_code }}" style="font-size:13px;">
                                    ...
                                    <td class="text-center">
                                        ...
                                        @else
                                            <select name="status_{{ $item->component_code }}"
                                                ...
                                                style="width:120px; margin:auto;" data-code="{{ $item->component_code }}"
                                                id="status-{{ $item->component_code }}">
                                                <option value="OK" {{ $item->status === 'OK' ? 'selected' : '' }}>
                                                    OK</option>
                                    ...
                                    <td style="min-width:220px;">
                                        ...
                                        @else
                                            <div id="damage-{{ $item->component_code }}"
                                                ...
                                                <input type="text" name="damage_remark_{{ $item->component_code }}"
                                                    class="form-control form-control-sm mb-1" placeholder="Damage remark..."
                                                    value="{{ $item->damage_remark ?? '' }}">
                                                <input type="file" name="photo_{{ $item->component_code }}"
                                                    class="form-control form-control-sm" accept="image/*">
                                                @if ($item->hasDamagePhoto())
                                                    <small class="text-success mt-1 d-block">
                                                        <i class="bi bi-check-circle"></i>
                                                        Foto: {{ $item->damage_photo }}
                                                    </small>
                                                @endif
                                            </div>
                                        @endif
        ...
        @if (!$inspection->isClosed())
            <div class="d-flex gap-2">
                <button type="submit" name="action" value="save" class="btn btn-outline-dark px-4">
